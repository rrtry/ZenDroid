...
class SchedulerFragment: Fragment(),
    ...
    /*
    private inner class AlarmViewHolder(private val alarmBinding: AlarmItemViewBinding) :
        RecyclerView.ViewHolder(alarmBinding.root),
        ViewHolderItemDetailsProvider<Long> {

        init {
            ViewCompat.setTransitionName(alarmBinding.startTime, SHARED_TRANSITION_START_TIME)
            ViewCompat.setTransitionName(alarmBinding.endTime, SHARED_TRANSITION_END_TIME)
            ViewCompat.setTransitionName(alarmBinding.scheduleSwitch, SHARED_TRANSITION_SWITCH)
            ViewCompat.setTransitionName(alarmBinding.clockViewSeparator, SHARED_TRANSITION_SEPARATOR)
        }
        ...
        private fun startDetailsActivity(alarmRelation: AlarmRelation) {
            startAlarmDetailsActivity(
                alarmRelation,
                createTransitionAnimationOptions())
        }

        fun onEditButtonClick(alarmRelation: AlarmRelation) {
            if (binding.recyclerView.isViewPartiallyVisible(itemView)) {
                binding.recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
                    ...
                    override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                        ...
                        Handler(Looper.getMainLooper()).postDelayed({
                            binding.recyclerView.clearOnScrollListeners()
                            startDetailsActivity(alarmRelation)
                        }, 100)
                    }
                })
                binding.recyclerView.smoothScrollToPosition(bindingAdapterPosition)
            } else startDetailsActivity(alarmRelation)
        }
        ...
        fun bind(alarmRelation: AlarmRelation) {
            ...
            alarmBinding.scheduleSwitch.isChecked = alarm.isScheduled
            alarmBinding.startTime.text = TextUtil.formatLocalTime(requireContext(), alarm.startTime)
            alarmBinding.endTime.text = TextUtil.formatLocalTime(requireContext(), alarm.endTime)
            alarmBinding.occurrencesTextView.text = TextUtil.formatWeekDays(alarm.scheduledDays)
            alarmBinding.profileName.text = "${alarmRelation.startProfile} - ${alarmRelation.endProfile}"

            alarmBinding.scheduleSwitch.setOnCheckedChangeListener { buttonView, isChecked ->
            ...
            alarmBinding.deleteAlarmButton.setOnClickListener {
                viewModel.removeAlarm(alarmRelation)
            }
            alarmBinding.editAlarmButton.setOnClickListener {
                onEditButtonClick(alarmRelation)
            }
        }
    }
            ...
            if (payloads.isNotEmpty()) {
                payloads.forEach { i ->
                    when (i) {
                        ...
                        SELECTION_CHANGED_MARKER -> Animations.selected(holder.itemView, tracker.isSelected(getItem(position).alarm.id))
                    }
                }
